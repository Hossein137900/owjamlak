...
export async function POST(request: NextRequest) {
  try {
    ...
    const topConsultantsDir = join(
      process.cwd(),
      "public",
      "uploads",
      "topConsultants"
    );
    if (!existsSync(topConsultantsDir)) {
      await mkdir(topConsultantsDir, { recursive: true });
    }

    // Generate filename
    const extension = imageFile.name.split(".").pop() || "webp";
    const filename = `consultant_${Date.now()}.${extension}`;
    const filepath = join(topConsultantsDir, filename);

    // Save file
    const bytes = await imageFile.arrayBuffer();
    await writeFile(filepath, Buffer.from(bytes));

    const imageUrl = `/api/consultant-champion/images/${filename}`;
