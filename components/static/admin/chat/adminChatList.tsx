...
export default function ChatAdminList() {
  ...
  useEffect(() => {
      ...
      if (data.room) {
        setActiveRooms((prev) => {
          ...
          if (newRooms.has(data.room!)) {
            ...
            const messageExists = roomData.messages.some(msg => 
              msg.text === data.text && msg.time === data.time && msg.name === data.name
            );
            if (!messageExists) {
    ...
    newSocket.on("adminMessageUpdate" as any, (data: Message & { room?: string }) => {
      if (data.room) {
        setActiveRooms((prev) => {
          const newRooms = new Map(prev);
          if (newRooms.has(data.room!)) {
  ...
  const loadRecentChats = async (): Promise<void> => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SOCKET_URL || "http://localhost:3500"}/api/chat-sessions/recent?limit=5`
      );
      const sessions: ChatSession[] = await response.json();
      sessions.forEach((session) => {
        createRoomCard(session.sessionId, session.userName, session.hasUnreadMessages);
      });
    } catch (error) {
  ...
  const loadMoreHistory = async (): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SOCKET_URL || "http://localhost:3500"}/api/chat-sessions/history?skip=${skip}&limit=5`
      );
      const sessions: ChatSession[] = await response.json();
  ...
  const createRoomCard = (sessionId: string, userName: string, hasUnread: boolean = false): void => {
    setActiveRooms((prev) => {
      const newRooms = new Map(prev);
  ...
  const loadRoomHistory = async (roomName: string): Promise<void> => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SOCKET_URL || "http://localhost:3500"}/api/messages/${roomName}`
      );
        ...
        <div className="w-80 bg-gray-800/50 border-r border-gray-700 flex flex-col">
          <div className="p-6 border-b border-gray-700">
            ...
            <p className="text-gray-400 text-sm mt-1">
              {activeRooms.size} conversations
            </p>
          </div>
              ...
              <>
                {Array.from(activeRooms.entries())
                  .sort(([,a], [,b]) => {
                    ...
                    if (a.hasNewMessage !== b.hasNewMessage) return b.hasNewMessage ? 1 : -1;
                    return 0;
                  })
                  .map(([roomName, roomData]) => (
                    <div
                      ...
                      className={`p-4 border-b border-gray-700/50 cursor-pointer hover:bg-gray-700/30 transition-all ${
                        selectedRoom === roomName ? "bg-blue-600/20 border-l-4 border-l-blue-500" : ""
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <h3 className="text-white font-medium truncate">{roomData.userName}</h3>
                            <div className={`w-2 h-2 rounded-full ${
                              roomData.isOnline ? 'bg-green-400' : 'bg-gray-500'
                            }`}></div>
                          </div>
          ...
          {selectedRoom && selectedRoomData ? (
            <>
              ...
              <div className="p-4 bg-gray-800/50 border-b border-gray-700 flex justify-between items-center">
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-xl font-semibold text-white">{selectedRoomData.userName}</h2>
                    <div className={`w-3 h-3 rounded-full ${
                      selectedRoomData.isOnline ? 'bg-green-400' : 'bg-gray-500'
                    }`}></div>
                  </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {selectedRoomData.messages.map((msg, index) => (
                  <div
                    key={index}
                    className={`flex ${msg.name === "Admin" ? "justify-end" : "justify-start"}`}
                  >
                    <div
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-xs opacity-70 font-medium mr-3">{msg.name}</span>
                        <span className="text-xs opacity-70 whitespace-nowrap ml-auto">{msg.time}</span>
                      </div>
              ...
              <div className="p-4 bg-gray-800/50 border-t border-gray-700">
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) => updateInputValue(selectedRoom, e.target.value)}
                    onKeyPress={(e) => e.key === "Enter" && sendAdminMessage(selectedRoom)}
